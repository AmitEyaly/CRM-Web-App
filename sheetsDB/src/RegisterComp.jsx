...
const handleChange = (e) => {
  const { id, value } = e.target;
     const parsedValue = id === 'fullname' || id === 'password' || id === 'organization' ? value : value;
    setUser(prevUser => ({
        ...prevUser,
        [id]: parsedValue
    }));
...
const submitNewUser = async (e) => {
    ...
    try {
        ...
        const response = await axios.post('http://localhost:1913/users',
            user,
            {
        ...
        <div className="row" style={{ justifyContent: "center" }}>
          ...
          {/* <div className="col-7">
            <select className="form-select" aria-label="Default select example" required>
              <option selected style={{ textAlign: "right" }} >תפקיד</option>
              <option value="1">לקוח/ה</option>
              <option value="2">צוות</option>
            </select>
            <br />
          </div> */}
          <div className="col-7">
            <button type="text"
              ...
              style={{ textAlign: "right" }}
              className="btn btn-success" >הרשמה</button>
          </div>
        </div>
      </form>
      <br />
      <br />
      <button><Link to="/login">כניסת משתמשים רשומים</Link></button>
    </div>
  )
}
